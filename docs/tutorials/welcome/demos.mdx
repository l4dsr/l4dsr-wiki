...
## SST
Recording demos changes slightly depending on whether you are using [SST](mikes.software/sst) or not. I'll briefly explain the differences:
...
## Demo recording basics (this assumes you have latest SST installed and loaded)
* To record a demo, write `record speedrun` in the developer console, either from the main menu, or ingame.
  * Demo recording must begin before you take control at the start of the run in order for the run to be valid.
...
* **If you stop and restart demo recording, the demo number/counter will reset, and it will automatically overwrite existing demo files.**
